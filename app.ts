        ...
        <div class="p-6 ${read ? 'bg-gray-400' : 'bg-white'} mt-6 rounded-lg shadow-md transition-colors duration-500 hover:bg-green-100">
          ...
          <div class="flex mt-3">
            <div class="grid grid-cols-3 text-sm text-gray-500">
              <div class="mr-5"><i class="fas fa-user mr-1"></i>${user}</div>
              <div class="mr-5"><i class="fas fa-heart mr-1"></i>${points}</div>
              <div class="mr-5"><i class="far fa-clock mr-1"></i>${time_ago}</div>
            </div>
          </div>
        </div>
      `);
...
class NewsDetailView extends View {
  ...
  private makeComment(comments: NewsComment[]): string {
      ...
      if (comment.comments.length > 0) { // 삭제된 글은 미표시
        this.addHtml(`
          <div style="padding-left: ${comment.level * 40}px;" class="mt-4">
            ...
            <p class="text-gray-700">${comment.content.replace(/\$/g, "&#36;")}</p>
          </div>
        `);
